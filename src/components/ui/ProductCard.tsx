...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  // Genişletilmiş ürün (renge özel kart)
  const dp = product as DisplayProduct;
  const displayColor = dp.displayColor;

  // Bu kartta gösterilecek görseller — varyantı varsa onun fotoğrafları, yoksa ürünün ana fotoğrafları
  const cardImages = (displayColor && product.colorImages?.[displayColor]?.length)
    ? product.colorImages[displayColor]
    : product.images;
  const primaryImg = cardImages[0];
  const hoverImg = cardImages[1] || primaryImg;

  // Linke renk parametresi ekle — ürün sayfasında otomatik seçilecek
  const href = displayColor
        ...
        <Link href={href}>
          {primaryImg ? (
            <Image
              ...
              alt={displayColor ? `${product.name} - ${displayColor}` : product.name}
              fill
      ...
      <div className="flex flex-col gap-1 sm:gap-1.5 px-0.5">
        ...
        <Link href={href}>
          <h3 className="text-xs sm:text-sm font-light text-gray-900 line-clamp-1 group-hover:text-black transition-colors leading-relaxed">
            {product.name}
            {displayColor && (
              <span className="text-gray-500"> · {displayColor}</span>
            )}
          </h3>
        </Link>
