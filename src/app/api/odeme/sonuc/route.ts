  ...
  try {
    ...
    if (
      ...
      for (const item of order.items) {
        ...
        const updated = decrementStock(current, item.color, item.size, item.quantity);

        await prisma.product.update({
          where: { id: item.productId },
          data: {
            sizeStock: JSON.stringify(updated.sizeStock),
            colorSizeStock: JSON.stringify(updated.colorSizeStock),
            stock: stockLevel(totalStock(updated)),
          },
        });
      }
