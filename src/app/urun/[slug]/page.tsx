...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/bestsellers')
      .then(r => r.json())
      .then((data: Product[]) => setBestsellers(data.filter(p => p.slug !== slug).slice(0, 4)));
  }, [slug]);
  ...
  if (!product) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center">
        ...
        <Link href="/" className="text-sm underline">Anasayfaya Dön</Link>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="flex flex-col gap-5">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {product.sizes.map(size => {
                  const isDouble = /^\d{2}\/\d{2}$/.test(size);
                  const bedenNo = isDouble
                    ? (['38/40','42/44','46/48','50/52','54/56'].indexOf(size) + 1) + ' BEDEN'
                    : null;
                  return (
                  <button key={size} onClick={() => { setSelectedSize(size); setSizeError(false); }}
                    className={`flex flex-col items-center justify-center min-w-12 h-12 px-2 text-sm border transition-colors leading-tight ${
                      selectedSize === size ? 'border-black bg-black text-white' : 'border-gray-200 hover:border-gray-400'
                    }`}>
                    <span>{size}</span>
                    {bedenNo && <span className="text-xs opacity-70">{bedenNo}</span>}
                  </button>
                  );
                })}
              </div>
          ...
          <div className="flex gap-3">
            ...
            <a href={`https://wa.me/905000000000?text=Merhaba, "${product.name}" ürünü hakkında bilgi almak istiyorum.`}
